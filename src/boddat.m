...
{'R','V','X','A','RTN'},...5
{'number','cb','name','gm','radius','triaxial','j2','rotation_period'},...13
{'reference','date'},...15
...
elseif bvi<20;clear a;fn=params('bva');fn=fn(bvi,1:min(3,end));eval(['a=get' fn '(b);']);
% ii=~imag(b);
...
if rf;f=fopen(file);n=fread(f,2,'double')';di=fread(f,n(1:2),'double');fclose(f);%read from file if rf, 1st two entries is data size
    ...
    case 3;sav=params('save');params('save',0);[X td]=ephem1(bib,te);params('save',sav);%get H
